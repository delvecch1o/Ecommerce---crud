...
function Cart(props) {
    ...
    const handleIncrement = (cart_id,scope) => {
    ...
    if (cart.length > 0) {
                ...
                <tbody>
                    {cart.map((item, idx) => {
                        ...
                        return (
                            <tr key={idx}>
                                <td width="10%">
                                    <img src={`http://localhost:8000/${item.product.image}`} alt={item.product.name} width="50px"></img>
                                </td>
        ...
        cart_HTML = <div>
            <div className='card card-body py-5 text-center shadow-sm'>
                <h4>Your Shopping Cart is Empay</h4>
            </div>
        </div>
